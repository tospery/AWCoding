...
static HTTPSessionManager *lInstance;
...
- (RACSignal *)handleResponse:(NSDictionary *)response class:(Class)cls tag:(NSInteger)tag isList:(BOOL)isList {
    if (![response isKindOfClass:[NSDictionary class]] &&
        ![response isKindOfClass:[NSArray class]]) {
        NSError *error = [NSError jx_errorWithCode:JXErrorCodeServerException];
        //error.jxObj4Tag = JXStrWithInt(tag);
...
- (id)adaptResponse:(id)response tag:(NSInteger)tag {
    id ret = response;
...
+ (instancetype)sharedInstance {
    if (nil == lInstance) {
        ...
        lInstance = [[HTTPSessionManager alloc] initWithBaseURL:baseURL];
    }
    return lInstance;
}
@end
